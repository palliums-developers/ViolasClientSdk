...
void deploy_tokens(client_ptr client, string script_files_path)
{
    ...
    for (size_t i = 0; i < ACCOUNT_NUM; i++)
    {
        ...
        token_mgr->mint(i, index, address, 1 * MICRO_LIBRO_COIN);

        cout << "Token index : " << i
             << ", owner's address : " << address
             << " , owner's balance is " << (double)token_mgr->get_account_balance(i, address) / MICRO_LIBRO_COIN
             << endl;
    }
}
...
void transfer_token(client_ptr client, string script_files_path)
{
    ...
    do
    {
        ...
        cout << "The original balance of token " << STABLE_TOKEN_NAMES[token_index]
             ...
             << (double)token_mgr->get_account_balance(token_index, receiver) / MICRO_LIBRO_COIN
             << endl;

        token_mgr->mint(token_index, account_index, receiver, amount * MICRO_LIBRO_COIN);
